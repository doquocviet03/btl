            ...
            <div class="grid gap-10 lg:grid-cols-[1.15fr_.65fr_.5fr]">
                ...
                <div>
                    ...
                    <div class="mt-4 grid gap-2 text-sm font-bold text-zinc-400">
                        <a class="hover:text-white" href="{{ route('public.dich-vu.index') }}">Dịch vụ cắt tóc nam</a>
                        <a class="hover:text-white" href="{{ route('public.tho-cat-toc.index') }}">Đội ngũ barber</a>
                        <a class="hover:text-white" href="{{ route('public.san-pham.index') }}">Sản phẩm chăm sóc tóc</a>
                        <a class="hover:text-white" href="{{ route('public.blog.index') }}">Blog kiểu tóc nam</a>
                        <a class="hover:text-white" href="{{ route('public.cua-hang.index') }}">Hệ thống cửa hàng</a>
                    </div>
                </div>
                <div>
                    <h3 class="font-black text-white">Giờ mở cửa</h3>
                    <p class="mt-4 text-sm font-bold text-zinc-400">08:00 - 18:00<br>Thứ 2 - Chủ nhật</p>
                    <a href="{{ route('public.dat-lich.index') }}" class="bh-btn-primary mt-6 w-full">Đặt lịch</a>
                </div>
            </div>
        </div>
        <div class="flex flex-col justify-between gap-3 py-6 text-sm text-zinc-500 md:flex-row">
            <p>© {{ date('Y') }} Barber House. Premium barber booking website.</p>
